...
Run
---
    python3 fit_bandit_mid_rl_v3.py
"""
...
def _craving_reward_trace(rewards: np.ndarray) -> np.ndarray:
...
def _craving_embedded_mid(df_full: pd.DataFrame, trials: pd.DataFrame) -> np.ndarray:
    ...
    fp = fp.sort_values('position_in_bandit_stream')
    pos = fp['position_in_bandit_stream'].to_numpy()
    rt = fp['target_rt_ms'].to_numpy()
    cp = -(rt - rt.mean()) / rt.std()              # sign-flipped z(RT): faster = higher craving
    crav = np.zeros(len(T))
    for i, t in enumerate(T):
        prior = cp[pos < t]                        # food probes occurring before this trial
        crav[i] = prior[-1] if len(prior) else 0.0
    sd = crav.std()
...
def analyze_one_file(path: Path, md5: str) -> Tuple[dict, List[dict]]:
    ...
    trials["craving_reward_trace"] = _craving_reward_trace(trials["reward"].to_numpy())
