...
def build_qc_flags(s: Dict[str, float]) -> Tuple[List[str], bool, str]:
    ...
    # v16: with a fixed window, hit rate is a free-varying performance measure, not a
    # staircase target, so the "did the staircase hold hits near 0.667" band no longer
    # applies. A very low hit rate can still indicate the deadline was too hard, so we
    # keep the low-hit flag but drop the high-hit flag under a fixed window.
    if pd.notna(hr) and hr < MIN_MID_HIT_RATE:
        flags.append("low_mid_hit_rate")
    if not STAIRCASE_REMOVED and pd.notna(hr) and hr > MAX_MID_HIT_RATE:
