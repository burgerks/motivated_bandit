...
M4 is the vigor-modulated-learning model. The positive learning rate moves with a
trial-wise craving proxy, alpha_pos_t = alpha_pos + phi * craving_t, where craving_t
is a causal, z-scored leaky trace of recent food-win outcomes (every bandit win shows
a food image). phi is the key parameter: phi > 0 means craving speeds value updating.
At phi = 0, M4 reduces exactly to M2, so the M4-vs-M2 likelihood-ratio test (phi_lrt_p)
is a clean 1-df per-subject test of vigor-modulated learning; a permutation option is
provided because the chi-square LRT is mildly anti-conservative at this trial count.
...
2. bandit_rl_trialwise_values.csv    (per-trial Q, PE, alpha_used, choice probs [M2],
                                      plus craving_t and the M4 dynamic learning rate)
...
Run
---
    python3 fit_bandit_mid_rl_v2.py
"""
...
# modulated-learning parameter; at phi = 0 the model is exactly M2.
FIT_M4 = True
# Trial-wise craving proxy. 'reward_trace' is a causal leaky accumulator of PAST
# food-win outcomes, z-scored within the run (so phi is delta-alpha per SD of craving
# and alpha_pos is the mean-craving learning rate).
CRAVING_METHOD = 'reward_trace'
CRAVING_DECAY = 0.6                    # leak per trial for the reward trace (0-1)
...
def _compute_craving(rewards: np.ndarray) -> np.ndarray:
    """Trial-wise craving proxy entering the learning-rate bias. 'reward_trace' is a
    causal leaky accumulator of PAST food-win outcomes (c_t depends only on trials
    before t, so it is a valid regressor for the update at t), then z-scored within the
    run. Returns zeros if the trace has no variance (e.g. all-loss run)."""
    if CRAVING_METHOD != 'reward_trace':
        raise ValueError(f'Unknown CRAVING_METHOD: {CRAVING_METHOD}')
    n = len(rewards)
...
def prepare_bandit_trials(df: pd.DataFrame) -> pd.DataFrame:
    ...
    b["craving_t"] = _compute_craving(b["reward"].to_numpy())   # M4 learning-rate modulator
...
def replay_m4(params, trials):
    ...
    crav = trials['craving_t'].to_numpy()
    rows = []
...
def fit_model_m4(trials, craving=None, n_random=None):
    ...
    if n < MIN_TRIALS_FOR_FIT:
        return {'model': 'M4', 'n_trials': n, 'k': k, 'optimizer_success': False,
                'optimizer_message': 'Too few trials'}
    if craving is None:
        craving = trials['craving_t'].to_numpy()
    if n_random is None:
    ...
    data = (trials['arm'].to_numpy(), trials['reward'].to_numpy(), np.asarray(craving, dtype=float))
...
def phi_permutation_p(trials, phi_abs_obs):
    """Permutation p-value for phi: shuffle the craving trace (breaking any craving->
...
def analyze_one_file(path: Path, md5: str) -> Tuple[dict, List[dict]]:
    ...
    # M4: craving-modulated learning rate. phi is the vigor-modulated-learning metric;
    # phi_lrt_p is the 1-df M4-vs-M2 likelihood-ratio test (M4 nests M2 at phi = 0).
    if FIT_M4:
        m4 = fit_model_m4(trials)
        fits['M4'] = m4
        bflags += boundary_flags(m4, 'M4')
        for key in ["alpha_pos", "alpha_neg", "beta", "phi", "pseudo_r2",
                    "neg_log_likelihood", "aic", "bic"]:
            summary[f'M4_{key}'] = m4.get(key)
        summary['phi'] = m4.get('phi')
        lrt = (2.0 * (m2['neg_log_likelihood'] - m4['neg_log_likelihood'])
               if pd.notna(m4.get('neg_log_likelihood')) else np.nan)
        summary['phi_lrt_stat'] = lrt
        summary['phi_lrt_p'] = math.erfc(math.sqrt(max(lrt, 0.0) / 2.0)) if pd.notna(lrt) else np.nan
        summary['phi_perm_p'] = (phi_permutation_p(trials, abs(m4['phi']))
                                 if (N_PHI_PERM and pd.notna(m4.get('phi'))) else np.nan)
        summary['delta_aic_M4_minus_M2'] = m4['aic'] - m2['aic'] if pd.notna(m4.get('aic')) else np.nan
        summary['delta_bic_M4_minus_M2'] = m4['bic'] - m2['bic'] if pd.notna(m4.get('bic')) else np.nan
        summary['craving_method'] = CRAVING_METHOD
    ...
    if SAVE_TRIALWISE and not pd.isna(m2.get("alpha_pos", np.nan)):
        ...
        # Attach the craving proxy (always) and the M4 dynamic gain rate (if M4 fit).
        m4fit = fits.get('M4', {}) if FIT_M4 else {}
        m4rows = replay_m4(m4fit['params'], trials) if 'params' in m4fit else None
        for i, r in enumerate(rows):
            r['craving_t'] = float(trials['craving_t'].iloc[i])
            if m4rows is not None:
                r['alpha_pos_dynamic_m4'] = m4rows[i]['alpha_pos_dynamic_m4']
...
def make_data_dictionary(summary_cols=None, trialwise_cols=None) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("M3_aic", "M3 AIC (k=4)."),
        ("M3_bic", "M3 BIC (k=4)."),
        ("M4_alpha_pos", "M4 baseline gain learning rate at mean craving (alpha_pos in alpha_pos_t = alpha_pos + phi*craving_t)."),
        ("M4_alpha_neg", "M4 loss learning rate (static; only the gain rate is craving-modulated)."),
        ("M4_beta", "M4 softmax inverse temperature."),
        ("M4_phi", "M4 craving-modulation slope on the gain learning rate; the vigor-modulated-learning parameter. phi>0 = craving speeds updating."),
        ("M4_pseudo_r2", "M4 McFadden pseudo-R^2 vs a uniform 3-way guess."),
        ("M4_neg_log_likelihood", "M4 negative log-likelihood."),
        ("M4_aic", "M4 AIC (k=4)."),
        ("M4_bic", "M4 BIC (k=4)."),
        ("phi", "Primary vigor-modulated-learning estimate (= M4_phi)."),
        ("phi_lrt_stat", "Likelihood-ratio statistic 2*(NLL_M2 - NLL_M4); M4 nests M2 at phi=0."),
        ("phi_lrt_p", "One-df chi-square p-value for phi != 0 (M4 vs M2). Mildly anti-conservative; prefer phi_perm_p when available."),
        ("phi_perm_p", "Permutation p-value for phi from shuffling the craving trace (NaN unless N_PHI_PERM>0)."),
        ("delta_aic_M4_minus_M2", "M4 AIC minus M2 AIC. Negative => craving-modulated learning preferred by AIC."),
        ("delta_bic_M4_minus_M2", "M4 BIC minus M2 BIC. Negative => craving modulation preferred by BIC."),
        ("craving_method", "Trial-wise craving proxy used by M4 (e.g. reward_trace)."),
        ("craving_decay", "Leak-per-trial of the reward-trace craving proxy."),
        ...
        ("craving_t", "Trialwise: z-scored causal craving proxy (leaky trace of past food-win outcomes); the M4 learning-rate modulator and a candidate iEEG regressor."),
        ("alpha_pos_dynamic_m4", "Trialwise: M4 craving-biased gain learning rate on this trial, clip(alpha_pos + phi*craving_t)."),
        ("rt", "Trialwise: choice reaction time (s)."),
    ]
    defs = dict(rows)  # variable -> definition (text is the source of truth)
    order = list(summary_cols or []) + list(trialwise_cols or [])
    if not order:
