...
N_FOOD_MID, N_NEUTRAL_MID = 34, 20

# Between-subject reliability of the vigor index. 1.0 = original perfect-measurement.
VIGOR_RELIABILITY = 0.4
...
def simulate_vigor(rng, n_subjects, reliability):
    """Latent vigor plus the noisy index the model is actually fitted with.

    reliability R is the BETWEEN-SUBJECT reliability of the residualized vigor index,
    not the within-subject single-probe lambda. R=1 reproduces the original behaviour."""
    ...
    return pd.DataFrame({"subject": np.arange(n_subjects),
                         "vigor_true": v_true, "residual_food_vigor": v_obs})


def _unused_simulate_mid_rts(rng, n_subjects):
...
def run(args):
    ...
    for sc in SCENARIOS:
        for d in range(args.n_datasets):
            mid = simulate_vigor(rng, args.n_subjects, args.vigor_reliability)
            vig_true = mid["vigor_true"].to_numpy()      # generates the data
            vig = mid["residual_food_vigor"].to_numpy()  # what the model is fitted with
            S = len(vig)
            ND = np.zeros((S, nat_b.size))
            true_t = np.zeros((S, 3)); true_nat = np.zeros((S, 3))
            for i, v in enumerate(vig_true):
...
def parse():
    ...
    p.add_argument("--vigor-reliability", type=float, default=VIGOR_RELIABILITY,
                   dest="vigor_reliability")
