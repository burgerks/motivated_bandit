...
# -*- coding: utf-8 -*-
"""
Batch analysis for the vigor-modulated 3-arm bandit + embedded mini-MID task
(bandit_mid_task_v17_3.py output).

Pipeline version v6 (2026-07-30): version-number alignment to task v17.3 only.
Logic is unchanged from v5, so this is a clean baseline for the agreed fixes
(run-aware switch rate, per-run reversal QC, learned_above_chance demoted to
descriptive, vigor renaming, run added to the trial export, mean_regret rename)
to be committed on top.
...
Run
---
    python3 analyze_bandit_mid_v6.py
"""
...
# Self-contained: read the task CSVs from the data/ folder next to THIS script (the
# _task folder), so the analysis runs from one spot with no path editing. Search is
# recursive, so the per-run subfolders the task creates are picked up automatically.
# To analyze a different folder, replace this with an absolute Path(...).
...
# Task design constants. These mirror the CFG block in bandit_mid_task_v17_3.py and
# are used to define phases and reversal windows; change them only if the task did.
#
# v16: the session is two independent runs. Each run has one reversal and Q resets
# at the run boundary, so a "phase" is (run, pre/post-reversal) -> up to 4 phases.
N_RUNS_EXPECTED = 2
N_TRIALS_PER_RUN = 100
# One 1-indexed within-run reversal trial per run (run 1, run 2).
REVERSAL_TRIAL_BY_RUN = [54, 46]
# swap_count resets each run (v17), so within a file it maxes at the reversals in a
# single run. Use this per-run count for the "reached the reversal" QC check.
REVERSALS_PER_RUN = 1
# Legacy single-stream reversal list, used ONLY as a fallback for pre-v16 files that
# have no 'run' column. Do not use for v16 data.
REVERSAL_TRIALS_LEGACY = [69, 130]
N_TRIALS_EXPECTED = N_RUNS_EXPECTED * N_TRIALS_PER_RUN
N_BONUS_FOOD_EXPECTED = 48           # 24 per run x 2 runs
N_BONUS_NEUTRAL_EXPECTED = 22        # 11 per run x 2 runs

# Fixed response window (staircase removed in v16). Rail-detection QC is disabled;
# these are kept only so any pre-v16 file can still be described.
FIXED_WINDOW_MS = 550
STAIRCASE_REMOVED = True             # when True, skip staircase-rail and hit-rate-band QC
WIN_FLOOR_MS = 230                   # legacy staircase floor (pre-v16 files only)
WIN_CEIL_MS = 600                    # legacy staircase ceiling (pre-v16 files only)
...
# Learning gate: a subject must choose the optimal arm above chance across the WHOLE
# task (all usable trials). Judging the whole run rather than the final phase credits
# subjects who learned early and disengaged late; that late drop is caught separately
# by the disengagement flag below.
...
# Late disengagement / fatigue: flag when final-phase optimal choice falls at least
# this far below the subject's best earlier phase (learned, then faded).
...
MIN_MID_HIT_RATE = 0.40                  # pre-v16 staircase floor (staircase held hits near 0.667)
MIN_MID_HIT_RATE_FIXED = 0.10            # fixed window: only near-zero hits flags a too-hard deadline (advisory)
...
def split_trials(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "trial" in bandit.columns:
        ...
        # v16: trial index repeats across runs, so sort by run then trial.
        if "run" in bandit.columns:
            bandit["run"] = pd.to_numeric(bandit["run"], errors="coerce").fillna(1).astype(int)
            bandit = bandit.sort_values(["run", "trial"]).reset_index(drop=True)
        else:
    ...
    if "position_in_bandit_stream" in bonus.columns:
        ...
        if "run" in bonus.columns:
            bonus["run"] = pd.to_numeric(bonus["run"], errors="coerce").fillna(1).astype(int)
            bonus = bonus.sort_values(["run", "position_in_bandit_stream"]).reset_index(drop=True)
        else:
...
def _run_series(bandit: pd.DataFrame) -> pd.Series:
    """1-indexed run per bandit row; defaults to run 1 for pre-v16 files."""
    if "run" in bandit.columns and bandit["run"].notna().any():
        return pd.to_numeric(bandit["run"], errors="coerce").fillna(1).astype(int)
    return pd.Series(1, index=bandit.index)


def _per_run_reversal_reach(bandit: pd.DataFrame) -> Dict[str, float]:
    """Per-run reversal completion, for the v6 per-run reversal QC.

    swap_count resets each run, so the file-wide max hides a run that never reached
    its reversal. Return the number of runs present and the smallest per-run max
    swap_count across those runs; the QC then requires every run to reach its
    reversal. Pre-v16 files (no swap_count) return NaN and fall back to max_swap_count.
    """
    if "swap_count" not in bandit.columns or not bandit["swap_count"].notna().any():
        return {"n_runs_present": int(_run_series(bandit).nunique()),
                "min_run_swap_count": float("nan")}
    runs = _run_series(bandit)
    sc = bandit["swap_count"].fillna(0).astype(int)
    per_run_max = [int(sc[runs == r].max()) for r in sorted(runs.unique())]
    return {"n_runs_present": int(len(per_run_max)),
            "min_run_swap_count": int(min(per_run_max)) if per_run_max else 0}


def phase_of_trials(bandit: pd.DataFrame) -> pd.Series:
    """Return a globally 1-indexed reversal phase per trial across the whole session.

    v16: swap_count resets each run, so a within-run swap_count alone would label
    run 2 as phases 1-2 again. We combine run and within-run phase into a global
    index: run r, within-run phase p -> (r-1)*2 + p. With one reversal per run this
    gives phases 1,2 (run 1 pre/post) and 3,4 (run 2 pre/post).

    Pre-v16 files (no run column, swap_count spans the whole session) fall back to the
    original behaviour: swap_count + 1.
    """
    runs = _run_series(bandit)
    multi_run = runs.nunique() > 1
    if "swap_count" in bandit.columns and bandit["swap_count"].notna().any():
        within = bandit["swap_count"].fillna(0).astype(int) + 1     # 1 before rev, 2 after
        if multi_run:
            return (runs - 1) * 2 + within
        return within
    # Fallback with no swap_count: cut by the within-run reversal trial, per run.
    phase = pd.Series(1, index=bandit.index)
    if multi_run:
        for r in sorted(runs.unique()):
            idx = int(r) - 1
            rev = REVERSAL_TRIAL_BY_RUN[idx] if idx < len(REVERSAL_TRIAL_BY_RUN) else 10 ** 9
            base = idx * 2                                   # 0 for run 1, 2 for run 2
            in_run = runs == r
            phase = phase.mask(in_run, base + 1)             # pre-reversal -> base+1
            phase = phase.mask(in_run & (bandit["trial"] >= rev), base + 2)  # post -> base+2
        return phase
    edges = [0] + REVERSAL_TRIALS_LEGACY + [10 ** 9]
    t = bandit["trial"]
...
def win_stay_lose_shift(bandit: pd.DataFrame) -> Dict[str, float]:
    ...
    """
    b = bandit[bandit["usable"]].copy()
    b["_run"] = _run_series(bandit).loc[b.index]
    sub = b.sort_values(["_run", "trial"]).reset_index(drop=True)
    wins_repeat, wins_n, loss_switch, loss_n = 0, 0, 0, 0
    for i in range(len(sub) - 1):
        if sub.loc[i, "_run"] != sub.loc[i + 1, "_run"]:
            continue                                    # skip the run-boundary transition
...
def reversal_metrics(bandit: pd.DataFrame) -> Dict[str, float]:
    """Pre/post accuracy and perseveration around each reversal, computed WITHIN run.

    v16: there is one reversal per run, at the within-run trial index given by
    REVERSAL_TRIAL_BY_RUN. Windows are taken within the run so run 1's post-reversal
    window never bleeds into run 2. Output keys are numbered globally (rev1, rev2, ...)
    in run order, so downstream code that expects rev1_/rev2_ still works.

    Perseveration is the share of post-reversal choices landing on the position that
    was optimal on the last pre-reversal trial; that old-best position is no longer
    optimal because each reversal rotates all three arms.
    """
    out: Dict[str, float] = {}
    w = REVERSAL_WINDOW
    ...
    # Build the list of (run_value, within_run_reversal_trial) to iterate.
    if multi_run:
        rev_list = [(r, REVERSAL_TRIAL_BY_RUN[int(r) - 1])
                    for r in sorted(runs.unique())
                    if int(r) - 1 < len(REVERSAL_TRIAL_BY_RUN)]
    else:
        rev_list = [(1, rev) for rev in REVERSAL_TRIALS_LEGACY]

    for ri, (rv, rev) in enumerate(rev_list, start=1):
        b = bandit[runs == rv] if multi_run else bandit
        pre = b[(b["trial"] >= rev - w) & (b["trial"] < rev) & b["usable"]]
        post = b[(b["trial"] >= rev) & (b["trial"] < rev + w) & b["usable"]]
        pre_acc = prop_true(pre["is_optimal"]) if len(pre) else float("nan")
        post_acc = prop_true(post["is_optimal"]) if len(post) else float("nan")
        prev_row = b[(b["trial"] == rev - 1)]
...
def bandit_summary(bandit: pd.DataFrame) -> Dict[str, float]:
    ...
    out: Dict[str, float] = {
        ...
        # v6 fix: reversal QC is per-run. swap_count resets each run, so the max over
        # a whole file is 1 even when only ONE run reached its reversal. Report the
        # minimum per-run swap_count across the runs actually present, and how many
        # runs are present, so the QC can require EVERY run to reach its reversal.
        **_per_run_reversal_reach(bandit),
        ...
        # v6 rename: this is realized counterfactual regret (obtained-vs-best-realized
        # points per trial), not expected regret from the true arm probabilities.
        "mean_realized_counterfactual_regret": safe_mean(usable["regret"]),
    ...
    # Learning gate: whole-task optimal choice vs chance (1/3), one-sided binomial.
    # Judged over ALL usable trials so a subject who learned early but disengaged late
    # is still credited as a learner; the late drop is flagged separately below.
    opt = usable["is_optimal"].dropna()
    k_all, n_all = int(opt.sum()), int(len(opt))
    out["learn_n"] = n_all
    out["learn_optimal_prop"] = (k_all / n_all) if n_all else float("nan")
    out["learn_binom_p"] = binom_upper_tail_p(k_all, n_all, CHANCE_OPTIMAL_PROP) if n_all else float("nan")
    out["learned_above_chance"] = bool(n_all > 0 and out["learn_binom_p"] < LEARNING_ALPHA)
    # Late-disengagement / fatigue basis: final-phase optimal choice vs the subject's
    # best earlier phase. A large drop means they learned, then faded (or checked out).
    phase_opt = {}
    for p in sorted(pd.unique(phase.dropna())):
        vals = bandit.loc[(phase == p) & bandit["usable"], "is_optimal"].dropna()
        if len(vals):
            phase_opt[int(p)] = float(vals.mean())
    if phase_opt:
        last_p = max(phase_opt)
        final_opt = phase_opt[last_p]
        earlier = [v for p, v in phase_opt.items() if p < last_p]
        peak_earlier = max(earlier) if earlier else float("nan")
    else:
        final_opt = peak_earlier = float("nan")
    out["final_phase_optimal_prop"] = final_opt
    out["peak_earlier_phase_optimal_prop"] = peak_earlier
    out["late_optimal_drop"] = (peak_earlier - final_opt) if (pd.notna(peak_earlier) and pd.notna(final_opt)) else float("nan")
    # Exploration: switch rate and normalized entropy of the position choices.
    # v6 fix: switch rate is computed WITHIN run only. trial index repeats across
    # runs, so sorting by trial alone interleaved the two runs and manufactured a
    # spurious switch at nearly every step. Sort by (run, trial) and drop the single
    # run-boundary transition, matching win_stay_lose_shift. Entropy is order-free,
    # so it can still pool all choices across the session.
    ub = usable.copy()
    ub["_run"] = _run_series(bandit).loc[ub.index]
    switches, n_pairs = 0, 0
    for _, g in ub.groupby("_run"):
        cs = g.sort_values("trial")["choice"].dropna().tolist()
        switches += sum(cs[i] != cs[i - 1] for i in range(1, len(cs)))
        n_pairs += max(len(cs) - 1, 0)
    out["switch_rate"] = float(switches / n_pairs) if n_pairs > 0 else float("nan")
...
def post_probe_metrics(bandit: pd.DataFrame, bonus: pd.DataFrame) -> Dict[str, float]:
    ...
    v16: trial index repeats across runs, so the probe->trial match is keyed on
    (run, trial). A probe carries only onto bandit trials in the SAME run.
    """
    out: Dict[str, float] = {}
    b = bandit.copy()
    b["_run"] = _run_series(bandit).values
    by_trial = b.set_index(["_run", "trial"]) if "trial" in b.columns else pd.DataFrame()
    # Probe run labels, aligned to the same default as the bandit side.
    if "run" in bonus.columns and bonus["run"].notna().any():
        bonus_run = pd.to_numeric(bonus["run"], errors="coerce").fillna(1).astype(int)
    else:
        bonus_run = pd.Series(1, index=bonus.index)

    def get_trial(run_val, tnum):
        try:
            row = by_trial.loc[(run_val, tnum)] if (run_val, tnum) in by_trial.index else None
        except Exception:
            row = None
        # A duplicate (run,trial) should not occur, but guard by taking the first.
        if row is not None and isinstance(row, pd.DataFrame):
            row = row.iloc[0]
        return row
    ...
    for label, mask in [("food", bonus["food_bonus_cue"] == 1),
        ...
        sel = bonus.loc[mask]
        for idx, p in sel["position_in_bandit_stream"].dropna().astype(int).items():
            rv = int(bonus_run.loc[idx])
            nt = get_trial(rv, p + 1)
...
def build_qc_flags(s: Dict[str, float]) -> Tuple[List[str], bool, str]:
    ...
    # v6 fix: require EVERY run to reach its reversal. min_run_swap_count is the
    # smallest per-run max swap_count; if any run fell short it is below
    # REVERSALS_PER_RUN. A missing run (fewer than expected) also fails. Pre-v16
    # files have no per-run count (NaN) and fall back to the file-wide max.
    min_run_sc = s.get("min_run_swap_count")
    if pd.notna(min_run_sc):
        if min_run_sc < REVERSALS_PER_RUN or s.get("n_runs_present", 0) < N_RUNS_EXPECTED:
            flags.append("reversals_not_reached")
    elif s.get("max_swap_count", 0) < REVERSALS_PER_RUN:
        flags.append("reversals_not_reached")
    # Learning gate: judged over the WHOLE task (overall optimal choice vs chance), so
    # a subject who learned early but disengaged late is still credited as a learner.
    if s.get("learn_n", 0) > 0 and not s.get("learned_above_chance", False):
        flags.append("did_not_learn")
    # Late disengagement / possible fatigue: learned overall, but final-phase optimal
    # choice fell well below the subject's best earlier phase.
    if s.get("learned_above_chance", False) and pd.notna(s.get("late_optimal_drop")) \
            and s["late_optimal_drop"] >= MAX_LATE_OPTIMAL_DROP:
        flags.append("late_disengagement")
    ...
    if STAIRCASE_REMOVED:
        # Fixed window: hit rate is a performance measure driven partly by baseline
        # motor speed, not a QC target. A slow-but-valid subject responds on every
        # probe (late RTs are still logged via the grace window) yet can dip below the
        # old 0.40 band, so we do NOT flag on that band and it never drives exclusion.
        # Disengagement is caught by the no-response and premature flags and by the
        # usable-food-RT count. Only a near-zero hit rate is flagged, as an advisory
        # sign the deadline may be too hard for the group; it is not an exclusion.
        if pd.notna(hr) and hr < MIN_MID_HIT_RATE_FIXED:
            flags.append("mid_deadline_too_hard")
    else:
        # Pre-v16 staircase data: the staircase should have held hits in a band, so a
        # low or high hit rate indicates it failed to constrain the subject.
    ...
    # Two-tier premature check. Above MAX is exclusion-worthy; the (WATCH, MAX] band
    # raises an advisory "watch" flag to monitor whether the v14 too-soon deterrent
    # is holding anticipatory presses down (does not drive recommended_exclusion).
    if pd.notna(s.get("mid_premature_prop")):
        if s["mid_premature_prop"] > MAX_MID_PREMATURE_PROP:
            flags.append("high_mid_premature")
        elif s["mid_premature_prop"] > WATCH_MID_PREMATURE_PROP:
            flags.append("elevated_mid_premature")
    ...
    # Staircase rail flags apply only to pre-v16 (adaptive-window) data. Under a fixed
    # window every probe sits at the same value, so rail detection would fire for
    # everyone; skip it.
    if not STAIRCASE_REMOVED:
    ...
    # v6 fix: did_not_learn is descriptive only and no longer drives exclusion.
    # Gating on above-chance learning conditions on a behavioral outcome and can
    # induce collider/selection bias when learning is itself of interest. The flag
    # is still emitted for transparency; exclude on a prespecified rule instead.
...
def make_data_dictionary(summary_cols=None, phase_cols=None, cue_cols=None) -> pd.DataFrame:
    ...
    rows = [
        ...
        ("max_swap_count", "Highest swap_count reached anywhere in the file. swap_count resets each run, so this is a within-run count, not a session total. Use min_run_swap_count for per-run reversal QC."),
        ("n_runs_present", "Number of distinct runs present in the file (expected N_RUNS_EXPECTED)."),
        ("min_run_swap_count", "Smallest per-run max swap_count across the runs present. reversals_not_reached fires when this is below REVERSALS_PER_RUN or a run is missing, so every run must reach its reversal. NaN for pre-v16 files with no swap_count."),
        ...
        ("mean_realized_counterfactual_regret", "Mean per-trial realized counterfactual regret: best-realized-arm points minus obtained points across usable trials. This is not expected regret from the true arm probabilities."),
        ...
        ("learn_n", "Usable bandit trials used by the learning gate (whole task)."),
        ("learn_optimal_prop", "Overall optimal-choice accuracy across all usable trials; basis for the learning gate."),
        ("learn_binom_p", "One-sided binomial p-value that whole-task optimal choice exceeds chance (1/3)."),
        ("learned_above_chance", "True when learn_binom_p < LEARNING_ALPHA; False raises the did_not_learn exclusion flag."),
        ("final_phase_optimal_prop", "Optimal-choice accuracy in the final phase (for the late-disengagement check)."),
        ("peak_earlier_phase_optimal_prop", "Best optimal-choice accuracy among the phases before the final phase."),
        ("late_optimal_drop", "peak_earlier_phase_optimal_prop minus final_phase_optimal_prop; large values flag late disengagement/fatigue."),
