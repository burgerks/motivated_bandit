...
# -*- coding: utf-8 -*-
"""
Per-subject reinforcement-learning fits for the vigor-modulated 3-arm bandit
...
Logic is unchanged from v5.2, so this is a clean baseline for the agreed fixes
(vigor renaming, run added to the trialwise export, per-fit clipped-fraction
logging for alpha_pos(t)) to be committed on top.
...
Nested models are fit and compared by AIC/BIC:
    ...
    M4  M2 + modulator-biased positive learning rate (k = 4, adds phi)

M4 is the state-modulated-learning model: alpha_pos_t = alpha_pos + phi * modulator_t, so
the gain learning rate moves with a trial-wise state signal. phi > 0 means the modulator
speeds value updating, and at phi = 0 M4 reduces exactly to M2, so M4-vs-M2 is a clean
1-df test. M4 is fit under two modulator signals, reported side by side:
  embedded_mid (PRIMARY): the embedded MID food-cue vigor carried forward across the
     bandit. Each food probe contributes z(-target_rt) (faster = more vigor), held until
     the next probe. This is the designed implicit trial-wise vigor readout, the
     behavioral (RT-based) analog of the explicit momentary craving ratings used in
     Kulkarni et al. 2026. It is labeled vigor, not craving, because it is an implicit
     speeded-response measure with no concurrent craving report to validate that term.
  reward_trace (COMPETING): a causal leaky trace of recent bandit reward history; included
     so you can show the vigor signal predicts learning beyond simple reward history.
Per-subject phi is a diagnostic; with 24 food probes per run it is well powered only for
large effects, so the group-level (hierarchical) phi is the real test.

A third variant fits BOTH modulators jointly: alpha_pos_t = alpha_pos + phi_mid *
vigor_mid_t + phi_rew * reward_trace_t. Its decisive output is joint_phi_mid_lrt_p,
...
the ordinary reward-driven / associability learning-rate change, not vigor).
...
2. bandit_rl_trialwise_values.csv    (per-trial Q, PE, alpha_used, choice probs [M2],
                                      both modulator signals and the primary dynamic rate)
...
# --- M4: state-modulated learning rate (the alpha-bias / vigor model) --------
# M4 extends M2 with alpha_pos_t = alpha_pos + phi * modulator_t. phi is the state-
# modulated-learning parameter; at phi = 0 the model is exactly M2. Two modulator
# signals are fit and reported side by side (the first is primary):
...
#   'reward_trace' : causal leaky trace of recent bandit rewards (competing control).
FIT_M4 = True
MODULATOR_METHODS = ('embedded_mid', 'reward_trace')   # first is primary
REWARD_TRACE_DECAY = 0.6                    # leak per trial for the reward_trace signal (0-1)
PHI_BOUNDS = (-2.0, 2.0)              # bias slope bounds (the modulator is z-scored)
...
# A modulator-shuffling permutation test is more robust; set N_PHI_PERM > 0 (e.g. 500).
N_PHI_PERM = 0
...
def _reward_history_trace(rewards: np.ndarray, runs: np.ndarray = None) -> np.ndarray:
    """Competing modulator: causal leaky accumulator of recent bandit reward history
    (c_t depends only on trials before t), z-scored within the run. Zeros if flat.
    All task points buy food, so this is recent reward history, not a craving or
    associability signal, and is the control the vigor signal must beat.
    ...
    for i in range(n):
        ...
        acc = REWARD_TRACE_DECAY * acc + float(rewards[i])
    sd = c.std()
    return (c - c.mean()) / sd if sd > 0 else np.zeros(n)


def _vigor_embedded_mid(df_full: pd.DataFrame, trials: pd.DataFrame) -> np.ndarray:
    """Primary modulator: the embedded MID food-cue vigor carried forward across the
    bandit. Each food probe contributes z(-target_rt) (faster = more vigor); every
    ...
    # Sign-flipped z(RT) over ALL probes: faster = higher vigor, on a common scale.
    ...
    sd = crav.std()
    # NOTE (v6): this modulator is standardized TWICE, once at the probe level (cp
    # above) and again here on the carried-forward trial vector. The second scaling
    # ties phi's units to how long each state persists (probe spacing / carry
    # duration), so phi is not directly comparable across designs with different
    # probe density. This is intentional and internally consistent, but parameter-
    # and model-recovery simulations MUST reproduce the identical two-stage scaling
    # or recovered phi will be biased relative to the fitted phi.
...
def replay(params: np.ndarray, trials: pd.DataFrame, model: str) -> Tuple[List[dict], np.ndarray]:
    ...
    for _, t in trials.iterrows():
        ...
        rows.append({
            # v6 fix: carry run so repeated within-run trial numbers are unambiguous
            # (trial 1 exists in both runs). (run, trial) is the unique key downstream.
            "run": int(t["run"]) if ("run" in t and pd.notna(t["run"])) else 1,
...
# =============================================================================
# M4: STATE-MODULATED LEARNING RATE (alpha-bias model)
# =============================================================================


def neg_log_likelihood_m4(params, data):
    """M4 NLL: the M2 choice model with a modulator-biased positive learning rate,
    alpha_pos_t = clip(alpha_pos + phi * modulator_t). data = (arms, rewards, modulator)."""
    ...
    for i in range(arms.shape[0]):
        ...
        r = REWARD_VALUE if rews[i] == 1 else LOSS_VALUE
        apos_t = apos0 + phi * crav[i]                       # modulator-biased gain learning rate
        if apos_t < 1e-6:
            apos_t = 1e-6
        elif apos_t > 1 - 1e-6:
            apos_t = 1 - 1e-6
...
def replay_m4(params, trials, modulator_col='vigor_embedded_mid'):
    ...
    crav = trials[modulator_col].to_numpy()
    runs = trials["run"].to_numpy() if "run" in trials.columns else None
    rows = []
    for i in range(len(arms)):
        ...
        rows.append({'modulator_t': float(crav[i]), 'alpha_pos_dynamic_m4': float(apos_t)})
        q[c] += a_used * pe
    return rows


def fit_model_m4(trials, modulator=None, n_random=None):
    """Fit M4 (M2 + modulator-biased alpha_pos) by multistart L-BFGS-B."""
    n = len(trials)
    ...
    if modulator is None:
        modulator = trials['vigor_embedded_mid'].to_numpy()
    if n_random is None:
    ...
    data = (trials['arm'].to_numpy(), trials['reward'].to_numpy(),
            np.asarray(modulator, dtype=float), trials['run'].to_numpy())
    ...
    chance = n * math.log(3.0)
    x = best.x
    # v6: report the fraction of trials whose dynamic gain rate hit the (0,1) clip.
    # Heavy clipping flattens the phi likelihood and makes phi effects nonlinear
    # (a larger phi then moves only unclipped trials), so this flags fits whose phi
    # should be read with care.
    apos_t = x[0] + x[3] * np.asarray(modulator, dtype=float)
    clip_frac = float(np.mean((apos_t <= 1e-6) | (apos_t >= 1 - 1e-6))) if n else float('nan')
    return {'model': 'M4', 'n_trials': n, 'k': k,
            ...
            'neg_log_likelihood': nll, 'aic': aic, 'bic': bic,
            'pseudo_r2': 1.0 - nll / chance if chance > 0 else np.nan,
            'alpha_pos_clip_frac': clip_frac,
            'params': x, 'optimizer_success': bool(best.success), 'optimizer_message': str(best.message)}


def phi_permutation_p(trials, phi_abs_obs, modulator):
    """Permutation p-value for phi: shuffle the modulator signal (breaking any modulator->
    learning coupling), refit M4, and compare |phi_perm| to the observed |phi|."""
    rng = np.random.default_rng(RANDOM_SEED + 909)
    crav = np.asarray(modulator, dtype=float)
    hits = 0
    for _ in range(N_PHI_PERM):
        f = fit_model_m4(trials, modulator=rng.permutation(crav), n_random=PERM_STARTS)
...
def neg_log_likelihood_m4_joint(params, data):
    ...
    data = (arms, rewards, vigor_mid, reward_trace)."""
...
def fit_model_m4_joint(trials, cmid=None, crew=None, n_random=None):
    """Fit M4_joint (M2 + both modulators on alpha_pos) by multistart L-BFGS-B."""
    n = len(trials)
    ...
    if n < MIN_TRIALS_FOR_FIT:
        return {'model': 'M4_joint', 'n_trials': n, 'k': k, 'optimizer_success': False,
                'optimizer_message': 'Too few trials'}
    if cmid is None:
        cmid = trials['vigor_embedded_mid'].to_numpy()
    if crew is None:
        crew = trials['reward_history_trace'].to_numpy()
    if n_random is None:
        n_random = N_RANDOM_STARTS
...
def _bonus_hit_trace(df_full, trials, decay=None):
...
def analyze_one_file(path: Path, md5: str) -> Tuple[dict, List[dict]]:
    ...
    M2 trialwise rows augmented with modulator_t and the M4 dynamic learning rate."""
    df = pd.read_csv(path)
    ...
    # Trial-wise modulator signals for M4 (both fit below; embedded_mid is primary).
    trials = trials.copy()
    trials["vigor_embedded_mid"] = _vigor_embedded_mid(df, trials)
    trials["reward_history_trace"] = _reward_history_trace(
    ...
    # M4: state-modulated learning rate, fit under each modulator signal. phi is the
    # vigor-modulated-learning metric; {method}_phi_lrt_p is the 1-df M4-vs-M2 test.
    modulator_cols = {'embedded_mid': 'vigor_embedded_mid', 'reward_trace': 'reward_history_trace'}
    m4_by_method = {}
    if FIT_M4:
        for mth in MODULATOR_METHODS:
            cvec = trials[modulator_cols[mth]].to_numpy()
            m4 = fit_model_m4(trials, modulator=cvec)
            m4_by_method[mth] = m4
            fits[f'M4_{mth}'] = m4
            bflags += boundary_flags(m4, f'M4_{mth}')
            for key in ["alpha_pos", "alpha_neg", "beta", "phi", "pseudo_r2",
                        "neg_log_likelihood", "aic", "bic", "alpha_pos_clip_frac"]:
        ...
        primary = MODULATOR_METHODS[0]
        summary['primary_modulator'] = primary
        ...
        summary['reward_trace_decay'] = REWARD_TRACE_DECAY
        ...
        if 'embedded_mid' in m4_by_method and 'reward_trace' in m4_by_method:
            ...
            trials['bonus_hit_trace'] = _bonus_hit_trace(df, trials)
            full3 = fit_model_m4_multi(
                trials, ['vigor_embedded_mid', 'reward_history_trace', 'bonus_hit_trace'])
            ctrl2 = fit_model_m4_multi(
                trials, ['reward_history_trace', 'bonus_hit_trace'])
    ...
    if SAVE_TRIALWISE and not pd.isna(m2.get("alpha_pos", np.nan)):
        ...
        # Attach both modulator signals and the primary-method dynamic gain rate.
        primary = MODULATOR_METHODS[0]
        prim_fit = m4_by_method.get(primary, {})
        prim_rows = (replay_m4(prim_fit['params'], trials, modulator_cols[primary])
                     if 'params' in prim_fit else None)
        for i, r in enumerate(rows):
            r['vigor_embedded_mid'] = float(trials['vigor_embedded_mid'].iloc[i])
            r['reward_history_trace'] = float(trials['reward_history_trace'].iloc[i])
...
def make_data_dictionary(summary_cols=None, trialwise_cols=None) -> pd.DataFrame:
    ...
    rows = [
        ...
        # Trialwise export.
        ("run", "Trialwise: 1-indexed run (1 or 2). (run, trial) is the unique row key; trial numbers repeat across runs."),
        ("trial", "Trialwise: 1-indexed bandit trial number, within run (repeats across runs)."),
        ("swap_count", "Trialwise: reversals completed before this trial, within run (phase = swap_count + 1)."),
        ...
        ("vigor_embedded_mid", "Trialwise: z-scored embedded MID food-cue vigor carried forward (PRIMARY modulator, the vigor signal); candidate iEEG regressor."),
        ("reward_history_trace", "Trialwise: z-scored recent reward-history signal (competing modulator, not vigor)."),
        ("alpha_pos_dynamic_primary", "Trialwise: primary-method M4 modulator-biased gain learning rate, clip(alpha_pos + phi*primary_modulator_t)."),
    ...
    for mth in ("embedded_mid", "reward_trace"):
        ...
        rows += [
            (f"{mth}_alpha_pos", f"M4 [{role}]: baseline gain learning rate at mean modulator value."),
            (f"{mth}_alpha_neg", f"M4 [{role}]: loss learning rate (static)."),
            (f"{mth}_beta", f"M4 [{role}]: softmax inverse temperature."),
            (f"{mth}_phi", f"M4 [{role}]: modulator slope on the gain rate; phi>0 = the modulator speeds updating."),
            (f"{mth}_pseudo_r2", f"M4 [{role}]: McFadden pseudo-R^2."),
            (f"{mth}_alpha_pos_clip_frac", f"M4 [{role}]: fraction of trials whose dynamic gain rate alpha_pos(t) hit the (0,1) clip. High values flag a phi read with care (clipping flattens the likelihood and makes phi effects nonlinear)."),
            ...
            (f"{mth}_delta_aic_vs_M2", f"M4 [{role}]: AIC minus M2 AIC. Negative => modulation preferred."),
            (f"{mth}_delta_bic_vs_M2", f"M4 [{role}]: BIC minus M2 BIC."),
        ]
    rows += [
        ("primary_modulator", "Which modulator is mirrored to phi/phi_lrt_p/phi_perm_p (first in MODULATOR_METHODS); the vigor signal by default."),
        ("phi", "Primary vigor-modulated-learning estimate (mirror of <primary_modulator>_phi)."),
        ...
        ("reward_trace_decay", "Leak-per-trial of the reward_trace signal."),
    ]
    rows += [
        ("joint_alpha_pos", "M4_joint: baseline gain learning rate at mean modulator value (both modulators)."),
        ...
        ("bonus_hit_trace", "Trialwise: z-scored decaying accumulator of past food-probe hits (the +15 events); control for the RT-contingent bonus."),
        ...
        ("joint3_phi_mid_ctrl2_lrt_p", "DECISIVE bonus-clean test: 1-df p that embedded-MID vigor predicts alpha+ modulation beyond reward history AND the RT-contingent bonus."),
    ]
